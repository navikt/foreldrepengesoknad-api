...
public final class ForeldrepengerMapper {
    ...
    public static Søknad tilForeldrepengesøknad(ForeldrepengesøknadDto foreldrepengesøknad,
    ...
    private static Foreldrepenger tilYtelse(ForeldrepengesøknadDto f, List<VedleggDto> vedlegg) {
        return new Foreldrepenger(tilAnnenForelder(f.annenForelder()), tilRelasjonTilBarn(f.barn(), vedlegg),
                                  ...
                                  tilOpptjening(f.selvstendigNæringsdrivendeInformasjon(), f.frilansInformasjon(),
                                                f.andreInntekterSiste10Mnd(), vedlegg),
    ...
    static Rettigheter tilRettigheter(AnnenForelderDto annenforelder) {
        ...
        return new Rettigheter(annenpartsRettigheter.harRettPåForeldrepenger(), annenpartsRettigheter.erAleneOmOmsorg(),
    ...
    private static OverføringsPeriode tilOverføringsPeriode(OverføringsPeriodeDto u, List<VedleggDto> vedlegg) {
        return new OverføringsPeriode(u.fom(), u.tom(), u.årsak(), tilStønadskontoType(u.konto()), tilVedleggsreferanse(
                DokumentasjonReferanseMapper.dokumentasjonSomDokumentererUttaksperiode(vedlegg,
                                                                                       new ÅpenPeriodeDto(u.fom(),
                                                                                                          u.tom()))));
    }

    private static FriUtsettelsesPeriode tilFriUtsettelsesPeriode(UtsettelsesPeriodeDto u, List<VedleggDto> vedlegg) {
        return new FriUtsettelsesPeriode(u.fom(), u.tom(), u.erArbeidstaker(), u.årsak(), u.morsAktivitetIPerioden(),
                                         tilVedleggsreferanse(
                                                 DokumentasjonReferanseMapper.dokumentasjonSomDokumentererUttaksperiode(
                                                         vedlegg, new ÅpenPeriodeDto(u.fom(), u.tom()))));
    }

    private static UtsettelsesPeriode tilUtsettelsesPeriode(UtsettelsesPeriodeDto u, List<VedleggDto> vedlegg) {
        return new UtsettelsesPeriode(u.fom(), u.tom(), u.erArbeidstaker(), u.årsak(), u.morsAktivitetIPerioden(),
                                      tilVedleggsreferanse(
                                              DokumentasjonReferanseMapper.dokumentasjonSomDokumentererUttaksperiode(
                                                      vedlegg, new ÅpenPeriodeDto(u.fom(), u.tom()))));
    }
    ...
    private static OppholdsPeriode tilOppholdsPeriode(OppholdsPeriodeDto u, List<VedleggDto> vedlegg) {
        return new OppholdsPeriode(u.fom(), u.tom(), u.årsak(), tilVedleggsreferanse(
                DokumentasjonReferanseMapper.dokumentasjonSomDokumentererUttaksperiode(vedlegg,
                                                                                       new ÅpenPeriodeDto(u.fom(),
                                                                                                          u.tom()))));
    }
    ...
    private static UttaksPeriode tilUttaksPeriode(UttaksPeriodeDto u, List<VedleggDto> vedlegg) {
        return new UttaksPeriode(u.fom(), u.tom(), tilVedleggsreferanse(
                DokumentasjonReferanseMapper.dokumentasjonSomDokumentererUttaksperiode(vedlegg,
                                                                                       new ÅpenPeriodeDto(u.fom(),
                                                                                                          u.tom()))),
                                 tilStønadskontoType(u.konto()),
                                 u.ønskerSamtidigUttak() != null && u.ønskerSamtidigUttak(), u.morsAktivitetIPerioden(),
                                 u.ønskerFlerbarnsdager() != null && u.ønskerFlerbarnsdager(),
                                 u.samtidigUttakProsent() != null ? ProsentAndel.valueOf(
                                         u.samtidigUttakProsent()) : null, u.justeresVedFødsel());
    }

    private static GradertUttaksPeriode tilGradertUttaksperiode(GradertUttaksPeriodeDto u, List<VedleggDto> vedlegg) {
        return new GradertUttaksPeriode(u.fom(), u.tom(), tilVedleggsreferanse(
                DokumentasjonReferanseMapper.dokumentasjonSomDokumentererUttaksperiode(vedlegg,
                                                                                       new ÅpenPeriodeDto(u.fom(),
                                                                                                          u.tom()))),
                                        tilStønadskontoType(u.konto()),
                                        ...
                                        u.ønskerFlerbarnsdager() != null && u.ønskerFlerbarnsdager(),
                                        u.samtidigUttakProsent() != null ? ProsentAndel.valueOf(
                                                u.samtidigUttakProsent()) : null,
                                        u.stillingsprosent() != null ? ProsentAndel.valueOf(
                                                u.stillingsprosent()) : null,
                                        u.erArbeidstaker() != null && u.erArbeidstaker(),
                                        u.orgnumre(), true, u.erFrilanser(), u.erSelvstendig(), u.justeresVedFødsel());
    }
    ...
    private static UtenlandskForelder tilUtenlandskForelder(UtenlandskForelderDto utenlandsk) {
        return new UtenlandskForelder(utenlandsk.fnr().value(), utenlandsk.bostedsland(),
